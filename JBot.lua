...
["179201489363795968"] = true, ["178525861476040704"] = true, ["188832914639814656"] = true, ["182501713423499264"] = true } 
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>stats' then
		message.channel:sendMessage(string.format(' ``` Name: JBot#7214. Bot: Yes. Desc: Multipurpose bot used to moderate servers or provide fun commands for you and your server. Language: Lua. Framework: Luvit. Lib: Discordia. Lib Creator: Sinister Rectus. Region: World. Version: 1.5.3. Updated: Yes. Updates: No. ```, %s', message.author:getMentionString()))
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	print(cmd)
	if cmd == '>setgame' then
		client:setGameName(" >help | 3 servers")
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>admins' then
		message.channel:sendMessage(string.format(' ``` Admin List: 189513922808774656/Katyusha 178525861476040704/Unique, 188832914639814656/JBot, 182501713423499264/Kongou```, %s', message.author:getMentionString()))
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>new' then
		message.channel:sendMessage(string.format(' ``` Current Version 1.5.3: No longer can kick JBot with >kick <@JBot#7214> ```, %s', message.author:getMentionString()))
